namespace pxsim {
    ...
    export class Image extends RefObject {
        public static height: number = 12;
        public width: number;
        public data: number[];
        ...
        public get(x: number, y: number): number {
            if (x < 0 || x >= this.width || y < 0 || y >= 12) return 0;
            return this.data[y * this.width + x];
        }
        public set(x: number, y: number, v: number) {
            if (x < 0 || x >= this.width || y < 0 || y >= 12) return;
    ...
    export function createImageFromBuffer(data: number[]): Image {
        return new Image(data.length / 12, data);
    }
    ...
    export function createImageFromString(text: string): Image {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            let charStart = (charCode - 32) * 12;
            if (charStart < 0 || charStart + 12 > w) {
                charCode = " ".charCodeAt(0);
                charStart = (charCode - 32) * 12;
            }
    ...
    export function createFont(): Image {
        ...
        for (let c = 0; c < n; c++) {
            for (let row = 0; row < 12; row++) {
                let char = data[c * 12 + row];
                for (let col = 0; col < 12; col++) {
                    if ((char & (1 << col)) != 0)
                        font.set((c * 12 + 4) - col, row, 255);
                }
            }
        }
        return font;
...
namespace pxsim.ImageMethods {
    export function showImage(leds: Image, offset: number, interval: number) {
        ...
        board().ledMatrixState.animationQ.enqueue({
            ...
            frame: () => {
                if (first) {
                    leds.copyTo(offset, 12, board().ledMatrixState.image, 0)
                    first = false;
                    return true;
                }
                return false;
        ...
        board().ledMatrixState.animationQ.enqueue({
            ...
            frame: () => {
                ...
                if (stride > 0) {
                    ...
                    leds.copyTo(off, c, display, 12 - stride)
                } else {
...
namespace pxsim.basic {
    export function showNumber(x: number, interval: number) {
        ...
        else showLeds(leds, interval * 12);
    }
    ...
    export function showAnimation(leds: Image, interval: number): void {
        ImageMethods.scrollImage(leds, 12, interval);
    }
